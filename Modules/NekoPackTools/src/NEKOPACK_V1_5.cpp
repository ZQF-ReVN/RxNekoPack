...
namespace NekoPackTools
{
	namespace Pack
	{
		...
		NEKOPACK_V1_5::NEKOPACK_V1_5(const std::wstring& wsPack) : m_wsPack(wsPack), m_pDecTable(nullptr)
		{
			m_ifsPack = OpenFileBinaryStream(wsPack);
		}

		NEKOPACK_V1_5::~NEKOPACK_V1_5()
		{
			if (m_pDecTable) { delete[] m_pDecTable; }
		}


		void NEKOPACK_V1_5::Extract()
		{
			ReadTable();
			ReadIndex();
		...
		void NEKOPACK_V1_5::BufferDecode(uint32_t nType, uint32_t* pEnc, uint8_t* pTable, uint32_t nKey, uint32_t nSize)
		{
			uint32_t uint = 0;
		...
		void NEKOPACK_V1_5::GetKeyAndSize(uint32_t& nKey, uint32_t& nSize)
		{
			uint32_t key = 0;
			uint32_t size[2] = { 0 };
