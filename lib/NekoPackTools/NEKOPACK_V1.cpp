	...
	namespace Pack
	{
		...
		void NEKOPACK_V1::Extract()
		{
			...
			for (const auto& entry : m_vecIndex)
			{
				...
				std::string file_path = folder + (char*)entry.aResName;
				SaveFileViaPath(file_path.c_str(), buffer_ptr, entry.uiResSize);
				PrintInfo(entry);
			}
		}
		...
		void NEKOPACK_V1::PrintInfo(const NEKOPACK_V1_Entry& eEntry)
		{
